 ...
 * 
 * <p>
 * Implements use cases for creating a new game, executing a turn, and
 * retrieving the game state.
 * </p>
 * 
 * <p>
 * This service manages game state persistence via
 * {@link GameSessionRepository},
 * creates new game instances through {@link GameStateFactory}, and delegates
 * computer turn calculations to a {@link ComputeTurnUseCase} implementation.
 * </p>
 * 
 * <p>
 ...
 * </p>
 * <ul>
 * <li>Initialize a new game with configurable starting tokens and starting
 * player</li>
 * <li>Validate, apply, and update the game state for player turns</li>
 * <li>Automatically execute the computer's turn when it is the computer's
 * move</li>
 ...
 * <li>{@code defaultStartingPlayer} – the default player who starts the
 * game</li>
 * </ul>
 */
@Slf4j
...
public class GameService implements CreateGameUseCase, ExecuteTurnUseCase, GetGameUseCase {
    ...
    /**
     * Executes a player turn by validating it and updating the game state.
     * If the game continues and it's the computer's turn, the computer move is
     * applied automatically.
     *
     * @param command The player's move, including the game ID and number of tokens
     *                taken.
     * @throws InvalidTurnException if the submitted turn is not valid (e.g. too
     *                              many tokens)
     */
    @Override
    public void executeTurn(ExecuteTurnCommand command) {
        log.info("Turn received for game {}: {} tokens", command.gameId(), command.turn().getTakeTokens());
    ...
    /**
     * Applies a valid turn to the game state and updates the game accordingly:
     * - If the last token was taken, the game ends and the winner is determined.
    ...
    /**
     * Automatically performs the computer's turn if it's the computer's move and
     * the game hasn't ended.
     * If the computer's move results in the game ending, the winner is updated
     * accordingly.
     *
     * @param game The current game state
     */
